...
export default class HermesBackup {
  ...
  async save() {
    try {
      ...
      for (const colName of this.allCollectionsForBackup) {
        db[colName] = await this.db.collection(colName).find({}, {projection: {_id: 0}}).toArray();
      }
  ...
  async getLatestBackupEvent() {
    return this.db.collection('events')
      ...
      .sort({'content.idData.timestamp': -1}).next();
  }

  async isRestoreRequired() {
    for (const colName of this.requiredCollections) {
      const colArray = await this.db.collection(colName).find({}, {projection: {_id: 1}}).toArray();
