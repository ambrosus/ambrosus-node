...
import locationAssetSchema from '../../../src/validators/schemas/custom/ambrosus.asset.location.json';
...
describe(`Event Entry Validator - 'ambrosus.asset.location'`, () => {
  const validEntry = {
  ...
  const locationAssetValidator = new EventEntryValidator('ambrosus.asset.location',
    new JsonSchemaValidator(locationAssetSchema));

  const expectValidationError = (brokenEntry, errorMessage) => {
    const event = createEventWithEntries([brokenEntry]);
    expect(() => locationAssetValidator.validate(event))
  ...
  it('accepts valid entry', async () => {
    ...
    expect(() => locationAssetValidator.validate(event)).to.not.throw();
  });
