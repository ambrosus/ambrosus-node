    ...
    describe('additional criteria', () => {
      ...
      before(async () => {
        ...
        eventsSet = [
          ...
          await scenario.addEvent(1, 0, {timestamp: 3}),
          await scenario.addEvent(1, 0, {timestamp: 4}),
          await scenario.addEvent(0, 1, {timestamp: 5}),
          await scenario.addEvent(0, 1, {timestamp: 6}),
          await scenario.addEvent(0, 1, {timestamp: 7}),
          await scenario.addEvent(1, 1, {timestamp: 8}),
          await scenario.addEvent(1, 1, {timestamp: 9})
        ];
      ...
      it('with createdBy param returns events for selected creator', async () => {
        ...
        const ret = await expect(storage.findEvents({createdBy: targetCreatorAddress})).to.be.fulfilled;
      ...
      it('with all params provided returns events for selected asset and creator, from between selected timestamps and with requested paging', async () => {
        ...
        const ret = await expect(storage.findEvents({fromTimestamp: 1, toTimestamp: 4, assetId: targetAssetId, perPage: 2, page: 0, createdBy: targetCreatorAddress})).to.be.fulfilled;
