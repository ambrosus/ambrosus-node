...
// describe.skip('getConfigurationForFindEventsQuery', () => {
//   let repository;
...
//       const ret = await expect(storage.find({}, 10)).to.be.fulfilled;
...
//       const ret = await expect(storage.find({}, 5)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(100);
//       expect(ret.resultCount).to.equal(135);
...
//   describe.skip('additional criteria', () => {
//     let scenario;
//     let eventsSet;
...
//       const ret = await expect(storage.find({assetId: targetAssetId}, 10)).to.be.fulfilled;
...
//       const ret = await expect(storage.find({createdBy: targetCreatorAddress}, 10)).to.be.fulfilled;
...
//       const ret = await expect(storage.find({fromTimestamp: 4}, 10)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(6);
//       expect(ret.resultCount).to.equal(6);
...
//       const ret = await expect(storage.find({toTimestamp: 2}, 10)).to.be.fulfilled;
...
//       const ret = await expect(storage.find({fromTimestamp: 2, toTimestamp: 4}, 10)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(3);
//       expect(ret.resultCount).to.equal(3);
...
//       const ret = await expect(storage.find({perPage: 3}, 10)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(3);
...
//       const ret = await expect(storage.find({page: 2, perPage: 3}, 10)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(3);
//       expect(ret.resultCount).to.equal(10);
...
//       const ret = await expect(storage.find({fromTimestamp: 1, toTimestamp: 4, assetId: targetAssetId, perPage: 2, page: 0, createdBy: targetCreatorAddress}, 10)).to.be.fulfilled;
...
//       const targetAssetId = scenario.assets[0].assetId;
//       const ret = await expect(storage.find({data: {asset: targetAssetId}}, 1)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(3);
//       expect(ret.resultCount).to.equal(3);
...
//       const ret = await expect(storage.find({data: {geoJson: {locationLongitude : 0, locationLatitude: 0, locationMaxDistance: 1000}}}, 3)).to.be.fulfilled;
//       expect(ret.results).have.lengthOf(2);
//       expect(ret.resultCount).to.equal(2);
